...
class QueueHandler extends EventEmitter {
 ...
 if (!payload) throw new Error('empty ' + 'queue' + ' payload');
 return { 'queue': String(payload).slice(0, 200) };
 }
...
module.exports = { QueueHandler, run: (payload, opts) => new QueueHandler().run(payload, opts) };