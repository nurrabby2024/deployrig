// Preview handling for DeployRig.
...
class PreviewHandler extends EventEmitter {
 ...
 if (!payload) throw new Error('empty ' + 'preview' + ' payload');
 return { 'preview': String(payload).slice(0, 200) };
 }
...
module.exports = { PreviewHandler, run: (payload, opts) => new PreviewHandler().run(payload, opts) };